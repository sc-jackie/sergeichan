...
description: One-shot onboarding for a new project — code repo or non-code — into Jackie OS.
---
...
## Path A — code project (own git repo)
...
6. **Design system** (owned code repos — skip API-only / CLI-only / shared 🤝):
   ...
   - **Doc path** (first match):
     ...
     - UI planned but not scaffolded yet → `docs/design-system.md` at repo root (`{UI_PACKAGE}` = _TBD_, `{TOKEN_SOURCE}` = _TBD_)
...
10. **Linear** (optional, owned repos) — add a Linear project for tracking. Execute in Cursor. Do not Delegate to the retired Cyrus farm. The 3 label groups (`Pipeline`/`Stage`/`Type`) are workspace-shared.
...
## One-time prerequisite (Sergei, once ever — not per project)
...
## Workflows that pick up automatically (after onboarding)
