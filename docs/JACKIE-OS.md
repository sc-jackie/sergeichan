...
**Primary desk:** [bb](https://getbb.app) (MacBook · Hermes VPS · PC planned) — `~/Jackie-OS/docs/workflows/bb-workspace.md`. Work may also happen in standalone Cursor, Claude Code (incl. mobile), or by hand. **Git on `origin/main` is code truth**; the vault is PM truth. **No Cursor automation** (`merge-wrap-up` removed 2026-06-17). Three ways the vault stays current, in priority order:
