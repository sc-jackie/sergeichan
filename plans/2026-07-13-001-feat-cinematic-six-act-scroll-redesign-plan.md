...
*(Rev 3, 2026-07-14: the rev-2 full-Three.js build shipped overnight and was reverted by owner verdict — `main` revert `14bd8e1`. This plan returns to the rev-1 Canvas-2D architecture, now binding: **no Three.js in any form**. The existing hero/case engine is the reference implementation and stays byte-level intact wherever possible. Delivery is a **PR for owner review** — no direct merge to `main`.)*
...
- **Truth gate on all public data (rev-3 binding, learned the hard way).** Build agents fabricated investment counts, conference credits, and product descriptions in rev 2. Every number, name, credit, and description in `site/js/data/*.js` must be verifiably true or explicitly marked coming-soon; the orchestrator/owner checks every entry before a PR is opened. An empty section beats a fictional one.
- **Reuse inventory from the reverted branch** (`sergeitran/eve-87-…`, tip `3c3695f`) — renderer-agnostic pieces, cherry-pick/copy rather than rebuild: truth-gated `site/js/data/{about,path,capital,voice,work}.js`; `site/js/content.js` DOM renderers (+ readyState-guard init); editorial section/timeline/stream/voice CSS blocks and the nav-rail styles from `site/css/site.css`; Lenis+GSAP wiring from `site/js/scroll.js`; vendored `gsap.min.js`/`ScrollTrigger.min.js`/`lenis.min.js` + `fonts/` (Fraunces/Inter). Explicitly NOT reused: `stage.js`, `js/three/*`, `js/scenes/*` (WebGL), three vendor files.
- **Verification rig (rev-3 binding).** Playwright cannot judge canvas visuals reliably and embedded panes throttle rAF: visual verification = headless Chrome screenshots (`--headless=new --screenshot --virtual-time-budget=9000`) LOOKED AT by the verifying agent, plus the orchestrator's own review of a saved capture set per act, plus the Vercel PR preview URL for real-browser owner review. Gotchas doc: `docs/solutions/frontend/webgl-overnight-build-gotchas.md` §2–4 apply to Canvas too.
