# Future functions & improvements — idea catalog

> **Idea backlog, not committed scope.** Candidate features and polish to pull from
> later. The owner prioritizes; nothing here is committed until it becomes a dated
> plan. Shipped work — [roadmap index](roadmap-index.md) and [`plans/archive/`](archive/).
>
> **Maintenance:** after any ship covering a line here, agents remove or **🔵**-refine it and bump
> [roadmap-index.md](roadmap-index.md) in the same commit
> ([wrap-up § Catalog cleanup](~/Jackie-OS/System/skills/wrap-up/SKILL.md#catalog-cleanup)).

Created 2026-07-11 at project onboarding.

---

## Core product

- 🔵 **Real screenshots in case-study frames** (EVE-78) — *mostly addressed* (`1ceceaf`): case "How it looks" frames now render faithful live-coded product screens grounded in each repo (Newfin/B-Unit/Draw), not placeholders. Remaining gap only if literal PNG screenshots of the running apps are wanted on top of the live-coded versions.
- **First essay: "Growing up between two cultures"** (EVE-79) — personal narrative/reflection content; becomes redesign act 5 (Voice) content
- **Custom domain** (EVE-80) — move from vercel.app to personal domain (DNS + Vercel configuration)

## Infra, ops & quality

- *(Seed ideas as they emerge)*

---

## Out of scope (v1)

- Blog/CMS system — stay static HTML/Canvas
- Animation frameworks — no standing dependency; add one only when a future approved concept cannot be expressed with the platform.
