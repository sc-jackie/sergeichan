# Ecosystem Architecture (/docs/architecture)
...
## 1. Jackie-OS — the AI operating system [#1-jackie-os--the-ai-operating-system]

Sergei's always-on OS. The **vault is memory**, **skills are the contract**, **scripts are dumb runners**, **Hermes is the 24/7 instance** on a VPS. &#x2A;*Operator desk:** Cursor + Tailscale + Hermes + Grokbot — [Operator desk](/jackie-os/docs/desk). History: [Desk journey](/jackie-os/docs/desk-journey). Tool-agnostic: any agent that opens `~/Jackie-OS` and reads `CLAUDE.md` *becomes* an instance. Hosts four code repos (Newfin, Rodyna, Draw, B-Unit) and multiple products/projects (B-Unit, Peaches Beauty, Nastusha), all synced to the vault via `project-sync`.
...
| **CTO**           | Code in `~/dev/{newfin,rodyna,draw}` via **Cursor**. Same agent, different hat.                  |
...
### Automation reality (2026-07-10) [#automation-reality-2026-07-10]
...
## 1b. LLM Gateway — subscription OAuth, no API billing (as of 2026-07-10) [#1b-llm-gateway--subscription-oauth-no-api-billing-as-of-2026-07-10]
...
## 2. Newfin — net-worth + financial advisor (`~/dev/newfin`) [#2-newfin--net-worth--financial-advisor-devnewfin]
...
## 3. Rodyna — family recipes (`~/dev/rodyna`) [#3-rodyna--family-recipes-devrodyna]
...
## 4. Draw — tennis community platform (`~/dev/draw`) [#4-draw--tennis-community-platform-devdraw]
...
* **Stack:** Next.js App Router (apps/web/) · React Native CLI (apps/mobile/) · Supabase Postgres · packages/shared (domain logic, draw algorithms, i18n) · FastAPI sidecar (services/auto\_post/) for cross-platform reel publishing · next-intl (en/ru/uk/pl/be) · Web Push (VAPID) + APNS · Sentry.
* **Status:** Active dev environment. Not yet in production. Morning-brief merge-poll skips Draw (read-only watcher).

***

## 4b. B-Unit — habit + quest app (`~/dev/bunit` + `Projects/B-Unit/`) [#4b-b-unit--habit--quest-app-devbunit--projectsb-unit]
...
* **Tracking:** Projects on **Linear** (`evegelin` workspace). Task state syncs on ship. The four-agent Cyrus farm is **retired** — execute in Cursor. History: [Desk journey](/jackie-os/docs/desk-journey).
...
## 5. Shared rails — same infra, different products [#5-shared-rails--same-infra-different-products]
